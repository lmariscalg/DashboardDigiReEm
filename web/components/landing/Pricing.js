      ...
      <div className="mx-auto max-w-7xl">
        ...
        <div className="mx-auto mt-14 grid max-w-7xl gap-6 lg:grid-cols-4">
          {plans.map((plan) => (
            <div
              key={plan.id}
              className={`relative flex flex-col rounded-2xl border border-[#D9A7B0] bg-white p-8 transition ${
                plan.highlighted
                  ? "shadow-[0_8px_28px_rgba(74,39,72,0.08)]"
                  : "hover:shadow-md"
              }`}
            >
              {plan.badge && (
                <span className="absolute -top-3.5 left-1/2 flex -translate-x-1/2 items-center gap-1.5 rounded-full bg-brand-gold px-3 py-1 text-xs font-semibold text-brand-navy ring-2 ring-white">
                  <Star className="size-3 fill-[#FAF8F5] text-brand-navy" strokeWidth={1.5} aria-hidden />
                  {plan.badge}
                </span>
              )}

              <h3 className="font-heading text-lg font-semibold text-brand-navy">{plan.name}</h3>
              <p className="mt-1 text-sm text-brand-muted">{plan.description}</p>

              <div className="mt-6 flex items-baseline gap-1">
                <span className="font-heading text-4xl font-semibold tracking-tight text-brand-navy">
              ...
              <Link
                ...
                className={`inline-flex w-full items-center justify-center rounded-full px-5 py-3 text-sm font-semibold transition-all duration-200 ${
                  plan.highlighted
                  ? "mt-8 bg-brand-navy text-[#FAF8F5] hover:opacity-90"
                  : "mt-8 border border-[#D9A7B0] text-brand-navy hover:border-brand-navy hover:bg-[#E8D8C8]"
                }`}
              >
                {plan.cta}
              </Link>
        ...
        {taxDisclaimer && (
          <p className="mt-8 text-center text-sm text-[#5E5B5D]">{taxDisclaimer}</p>
        )}

        {comparisonTable && (
          <div className="mt-16 md:mt-20">
            <div className="overflow-hidden rounded-2xl border border-[#E8D8C8] bg-white shadow-sm">
                ...
                <table className="w-full min-w-[640px] border-collapse text-sm">
                        ...
                        <th
                          ...
                          className={`px-3 py-4 text-center text-xs font-bold text-white md:px-4 md:py-5 md:text-sm ${
                            i === comparisonTable.highlightedColumn
                              ? "bg-[#D9A7B0]/20 ring-2 ring-inset ring-[#D9A7B0]/60"
                              : ""
                          }`}
                        >
                          {header}
                  ...
                  <tbody>
                    {comparisonTable.rows.map((row, rowIndex) => (
                      <tr
                        key={row.label}
                        className={rowIndex % 2 === 0 ? "bg-white" : "bg-[#FAF8F5]"}
                      >
                        <td className="border-t border-[#E8D8C8] px-4 py-3.5 text-left font-medium text-brand-navy md:px-6 md:py-4">
                          {row.label}
                        </td>
                        {row.values.map((value, colIndex) => (
                          <td
                            key={`${row.label}-${colIndex}`}
                            className={`border-t border-[#E8D8C8] px-3 py-3.5 text-center md:px-4 md:py-4 ${
                              colIndex === comparisonTable.highlightedColumn
                                ? "bg-[#D9A7B0]/15 ring-1 ring-inset ring-[#D9A7B0]/30"
                                : ""
                            }`}
                          >
